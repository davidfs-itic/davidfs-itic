# Arquitectura

Si seguim les indicacions de google i de molts altres desenvolupadors, veurem que qualsevol aplicació ha d'estar estructurada per "capes", o funcionalitats separades.

Vegeu com a referència aquest document: https://developer.android.com/topic/architecture
(Dins hi trobareu la Guide to app architecture)

Cursos (tutorial) de Android: https://developer.android.com/courses/pathways/android-architecture

Elements de l'arquitectura:

- [ViewModel](./viewmodel.md)
- [ViewModel de Settings (injecció manual)](./settingsviewmodel.md)
- [Observer](./observer.md)
- [Capa de Dades en Android](./capa_dades_android.md)
- [Capa de Dades amb Clean Architecture](./capa_dades_clean_architecture.md)
- [Inversió de Dependències](./inversiodependencies.md)
- [Patró Factory](./factory.md)
- [Exemple DIP recycler](./recyclerinversio.md) 
- [Principis SOLID](./solid.md)